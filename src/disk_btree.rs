...
impl DiskBtree {
    pub fn new(pool: Rc<RefCell<BufferPool>>, degree: usize) -> Self {
        let root_page_id = 0;
        let next_page_id = 1;
        ...
        tree.pool.borrow_mut().write_page(root_page_id, data);
        tree
    }

    pub fn open(pool: Rc<RefCell<BufferPool>>, root_page_id: u32, next_page_id: u32, degree: usize) -> Self {
        DiskBtree {
            pool,
            root_page_id,
            next_page_id,
    ...
    pub fn insert(&mut self, key: i32, value: Vec<u8>) {
        ...
        let page = self.pool.borrow_mut().get_page(root_id).clone();
        ...
        if keys.len() == max_keys {
            ...
            self.pool.borrow_mut().write_page(new_root_id, data);
    ...
    pub fn delete(&mut self, key: i32) {
        ...
        let page = self.pool.borrow_mut().get_page(self.root_page_id).clone();
    ...
    fn insert_non_full(&mut self, page_id: u32, key: i32, value: Vec<u8>) {
        let page = self.pool.borrow_mut().get_page(page_id).clone();
        let (is_leaf, mut keys, mut values, children) = deserialize_node(&page);
        ...
        if is_leaf {
            ...
            self.pool.borrow_mut().write_page(page_id, data);
        } else {
            let child_id = children[pos];
            let child_page = self.pool.borrow_mut().get_page(child_id).clone();
    ...
    fn split_child(&mut self, parent_id: u32, idx: usize) {
        let page = self.pool.borrow_mut().get_page(parent_id).clone();
        ...
        self.pool.borrow_mut().write_page(parent_id, data);
    }
    ...
    fn delete_key(&mut self, page_id: u32, key: i32) {
        ...
        if idx < keys.len() && keys[idx] == key {
            if is_leaf {
                ...
                let left_page = self.pool.borrow_mut().get_page(left_id).clone();
                let (_, left_keys, _, _) = deserialize_node(&left_page);

                let right_id = children[idx + 1];
                let right_page = self.pool.borrow_mut().get_page(right_id).clone();
    ...
    fn merge(&mut self, parent_id: u32, idx: usize) {
        ...
        self.pool.borrow_mut().write_page(left_id, data);
    ...
    fn fill(&mut self, parent_id: u32, idx: usize) {
        ...
        if idx > 0 {
            let left_sib = self.pool.borrow_mut().get_page(children[idx - 1]).clone();
        ...
        if idx < children.len() - 1 {
            let right_sib = self.pool.borrow_mut().get_page(children[idx + 1]).clone();
    ...
    fn borrow_from_prev(&mut self, parent_id: u32, idx: usize) {
        ...
        let sib_page = self.pool.borrow_mut().get_page(sib_id).clone();
        ...
        self.pool.borrow_mut().write_page(sib_id, data);
    }
